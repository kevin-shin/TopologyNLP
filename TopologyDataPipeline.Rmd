...
mainDataFrame <- data.frame(matrix(ncol = 3, nrow = 0))
columns <- c("bookID", "numHoles", "category")
  ...
  for (category in topics){
    ...
    for (value in bookShieldIDs){
      ...
      if (!is.null(bookDTM)){  
        ...
        print(numHoles)
        bookVector <- c(value, numHoles, category)
        rbind(mainDataFrame,bookVector)
      }
      else {
        bookVector <- c(value, 0, category)
        rbind(mainDataFrame,bookVector)
        print("0 here")
      }
    }
