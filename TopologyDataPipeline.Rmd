...
topics <- c("Children's Fiction", "Humor", "Science Fiction", "Adventure", "Horror")
limit <- 41

children <- isolateIDs("Children's Fiction")
humor <- isolateIDs("Humor")
science <- isolateIDs("Science Fiction")
...
###Approach 2
```{r}
collection <- c(7334, 7346, 770, 7493, 7862)
...
pamx <- pam(similarityTable, 1)
```

###Main function
```{r}

topics <- c("Humor", "Science Fiction")
#topics <- c("Children's Fiction", "Humor", "Science Fiction", "Adventure", "Reference", "Horror")
...
```{r}
#topics <- c("Children's Fiction", "Humor", "Science Fiction", "Adventure", "Reference", "Horror")
problematicMirrors <- function(category){
  for (category in topics){
    bookShieldIDs <- isolateIDs(category)
    for (value in bookShieldIDs){
        print(value)
        downloadedBook <- gutenberg_download(value)
    }
  }
}
...
compareStatistics <- function(dataFrame, statistic){
    df <- data.frame(matrix(0, ncol = 6, nrow = 6))
    colnames(df) <- topics
    rownames(df) <- topics
    for (row in rownames(df)){
      for (col in colnames(df)){
        ...
        significanceLevel <- t.test(mean(genreRowTable$statistic), mean(genreColTable$statistic))$p.value
