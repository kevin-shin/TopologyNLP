...
children <- sample(children, 5, replace=FALSE, prob=NULL)
humor <- sample(humor, 5, replace=FALSE, prob=NULL)
scifi <- sample(scifi, 5, replace=FALSE, prob=NULL)
adventure <- sample(adventure, 5, replace=FALSE, prob=NULL)
biographies <- sample(biographies, 5, replace=FALSE, prob=NULL)
      ...
      if (bookIndexVert > bookIndexHoriz){
        ...
        if (!is.null(bookDTM) && !is.na(otherBookDTM)){  
          ...
          DiagOther <- ripsDiag(X = otherBookDistanceMatrix,
                           ...
                           printProgress = FALSE, 
                           location=TRUE)
          wassersteinDist <- wasserstein(Diag[["diagram"]], DiagOther[["diagram"]], p = 0.5, dimension = 1)
